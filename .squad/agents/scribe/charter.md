...
**Worktree awareness:** Use the `TEAM ROOT` provided in the spawn prompt to resolve all `.ai-team/` paths.

After every substantial work session:

1. **Log the session** to `.ai-team/log/{YYYY-MM-DD}-{topic}.md`
2. **Merge the decision inbox** into `decisions.md` and delete inbox files
3. **Deduplicate and consolidate** overlapping decisions
4. **Propagate cross-agent updates** to affected history.md files
5. **Commit `.ai-team/` changes** using Windows-compatible commands
