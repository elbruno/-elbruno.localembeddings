...
- [20. ✅ Improvement Scenarios 1 & 2 (InMemoryVectorStore + FindClosest API)](#20--improvement-scenarios-1--2-inmemoryvectorstore--findclosest-api)
...
### 20. ✅ Improvement Scenarios 1 & 2 (InMemoryVectorStore + FindClosest API)

**Priority:** High | **Effort:** Medium | **Impact:** High | **Status:** ✅ Completed (2026-02-14)  
**Plan:** [plan_260214_1133.md](plan_260214_1133.md)

**Problem:** Semantic search and VectorData onboarding required repetitive boilerplate and external connector dependencies for an in-memory vector store experience.

**Implementation details (completed):**

- [x] **FindClosest convenience APIs (Scenario 2)**
  - **Files:**
    - `src/ElBruno.LocalEmbeddings/SemanticSearchResult.cs`
    - `src/ElBruno.LocalEmbeddings/EmbeddingGeneratorExtensions.cs`
    - `src/ElBruno.LocalEmbeddings/Extensions/EmbeddingExtensions.cs`
  - Added `SemanticSearchResult` and new `FindClosestAsync(...)` overloads (string + typed corpus), including optional precomputed embeddings, min score, top-K bounds, and deterministic ordering.
- [x] **Built-in InMemoryVectorStore (Scenario 1)**
  - **Files:**
    - `src/ElBruno.LocalEmbeddings.VectorData/InMemory/InMemoryVectorStore.cs`
    - `src/ElBruno.LocalEmbeddings.VectorData/InMemory/InMemoryVectorStoreCollection.cs`
    - `src/ElBruno.LocalEmbeddings.VectorData/InMemory/InMemoryVectorStoreRecordMetadata.cs`
    - `src/ElBruno.LocalEmbeddings.VectorData/Extensions/ServiceCollectionExtensions.cs`
  - Added a dependency-light in-memory `VectorStore` and collection implementation with reflection-cached metadata for `[VectorStoreKey]` and `[VectorStoreVector]`, plus cosine-similarity search.
- [x] **Test coverage and docs updates**
  - **Files:**
    - `tests/ElBruno.LocalEmbeddings.Tests/EmbeddingGeneratorFindClosestTests.cs`
    - `tests/ElBruno.LocalEmbeddings.Tests/EmbeddingExtensionsTests.cs`
    - `tests/ElBruno.LocalEmbeddings.VectorData.Tests/InMemoryVectorStoreTests.cs`
    - `tests/ElBruno.LocalEmbeddings.VectorData.Tests/ServiceCollectionExtensionsTests.cs`
    - `README.md`, `docs/getting-started.md`, `docs/api-reference.md`, `docs/vector-data-integration.md`, `samples/ConsoleApp/Program.cs`
  - Added deterministic/unit tests and updated docs/sample to the new one-line semantic-search and in-memory VectorData flow.
...
| **High** | 20. ✅ Improvement Scenarios 1 & 2 (InMemoryVectorStore + FindClosest API) | Medium | High | #4 VectorData Integration | Yes |
